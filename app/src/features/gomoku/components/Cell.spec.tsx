...
test('should render black stone', async ({ mount }) => {
  const component = await mount(
    <div className="w-10 h-10">
      <Cell x={0} y={0} value={PLAYER.BLACK} isLastMove={false} onClick={() => {}} />
    </div>
  );
...
test('should render white stone', async ({ mount }) => {
  const component = await mount(
    <div className="w-10 h-10">
      <Cell x={0} y={0} value={PLAYER.WHITE} isLastMove={false} onClick={() => {}} />
    </div>
  );
...
test('should highlight last move', async ({ mount }) => {
  const component = await mount(
    <div className="w-10 h-10">
      <Cell x={0} y={0} value={PLAYER.BLACK} isLastMove={true} onClick={() => {}} />
    </div>
  );
